...
func (c *Connection) Handle(ctx context.Context, wg *sync.WaitGroup) {
	...
	log.Printf("玩家 %d: 连接处理开始", c.playerID)
...
func (c *Connection) Close() {
	...
	// 从服务器移除玩家
	if c.playerID >= 0 {
		c.server.removePlayer(c.playerID)
	}

	log.Printf("玩家 %d: 连接已关闭", c.playerID)
}
...
func (c *Connection) sendLoop(ctx context.Context, wg *sync.WaitGroup) {
	...
	log.Printf("玩家 %d: 发送循环启动", c.playerID)

	for {
		select {
		case <-ctx.Done():
			log.Printf("玩家 %d: 发送循环停止", c.playerID)
			return
		...
		case data, ok := <-c.sendChan:
			...
			if err := binary.Write(c.conn, binary.BigEndian, length); err != nil {
				log.Printf("玩家 %d: 发送长度失败: %v", c.playerID, err)
				c.Close()
				return
			}
			...
			if _, err := c.conn.Write(data); err != nil {
				log.Printf("玩家 %d: 发送数据失败: %v", c.playerID, err)
				c.Close()
				return
			}
		}
...
func (c *Connection) receiveLoop(ctx context.Context, wg *sync.WaitGroup) {
	...
	log.Printf("玩家 %d: 接收循环启动", c.playerID)

	for {
		select {
		case <-ctx.Done():
			log.Printf("玩家 %d: 接收循环停止", c.playerID)
			return
		...
		default:
			...
			if err := binary.Read(c.conn, binary.BigEndian, &length); err != nil {
				if err != io.EOF {
					log.Printf("玩家 %d: 读取长度失败: %v", c.playerID, err)
				}
				c.Close()
				return
			}
			...
			if length > MaxPacketSize {
				log.Printf("玩家 %d: 消息过大 (%d bytes)", c.playerID, length)
				c.Close()
				return
			}

			if length == 0 {
				log.Printf("玩家 %d: 收到空消息", c.playerID)
				continue
			}
			...
			if _, err := io.ReadFull(c.conn, data); err != nil {
				log.Printf("玩家 %d: 读取数据失败: %v", c.playerID, err)
				c.Close()
				return
			}
			...
			if err := c.handleMessage(data); err != nil {
				log.Printf("玩家 %d: 处理消息失败: %v", c.playerID, err)
			}
		}
	}
}
...
func (c *Connection) String() string {
	if c.playerID >= 0 {
		return fmt.Sprintf("Connection{%d, %s}", c.playerID, c.conn.RemoteAddr())
	}
